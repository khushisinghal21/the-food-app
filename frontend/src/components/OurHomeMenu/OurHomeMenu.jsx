...
const OurHomeMenu = () => {
    ...
    // Simplified cart quantity check
...
// Replace your increment/decrement handlers with these:
const handleIncrement = (item) => {
    const currentQuantity = getQuantity(item._id);
    const cartItem = getCartItem(item._id);
    if (currentQuantity > 0 && cartItem) {
        console.log('[INCREMENT] updateQuantity', cartItem._id, currentQuantity + 1);
        updateQuantity(cartItem._id, currentQuantity + 1);
    } else {
        console.log('[INCREMENT] addToCart', item);
        addToCart(item, 1);
    }
};
...
const handleDecrement = (item) => {
    const currentQuantity = getQuantity(item._id);
    const cartItem = getCartItem(item._id);
    if (currentQuantity > 1 && cartItem) {
        console.log('[DECREMENT] updateQuantity', cartItem._id, currentQuantity - 1);
        updateQuantity(cartItem._id, currentQuantity - 1);
    } else if (currentQuantity === 1 && cartItem) {
        console.log('[DECREMENT] removeFromCart', cartItem._id);
